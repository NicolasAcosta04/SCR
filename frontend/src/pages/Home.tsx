...
const Home = () => {
  ...
  const fetchArticles = async (pageNum: number = 1, shouldAppend: boolean = true) => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/articles/fetch', {
        ...
        body: JSON.stringify({
          query: 'technology OR science OR business OR health',
          page_size: 10,
          days_back: 7,
        }),
      });
  ...
  useEffect(() => {
    fetchArticles(1, false); // Initial load without appending
  }, []);
  ...
  const handleScroll = () => {
    if (window.innerHeight + document.documentElement.scrollTop === document.documentElement.offsetHeight) {
      if (!loading && hasMore) {
        setPage((prev) => prev + 1);
        fetchArticles(page + 1, true); // Append new articles when scrolling
      }
    }
  };
